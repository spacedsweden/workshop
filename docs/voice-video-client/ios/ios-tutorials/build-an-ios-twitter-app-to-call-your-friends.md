...
description: >-
  Learn how to make an app that lets users call people who they follow on Twitter using Sinch app-to-app calling.
redirectFrom:
  - /docs/build-an-ios-twitter-app-to-call-your-friends
---
...
You should get an error saying that we'ven’t defined `fillTable`. Now is a great time to define it, so add the following method stub:
...
Great\! Now we've a call variable for the call screen, as well as a way to set it. Go back to **AppDelegate.m** and add the following method to handle incoming calls:
